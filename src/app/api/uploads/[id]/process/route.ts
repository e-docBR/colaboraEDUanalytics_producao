...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      // Execute the database operations inside a single transaction to ensure ACID compliance
      const { school, schoolClass, studentsCreated, studentsUpdated } = await db.$transaction(async (tx) => {
        ...
        if (school && result.class.grade) {
          schoolClass = await tx.schoolClass.upsert({
            where: {
        ...
        if (schoolClass) {
          ...
          const previousUploads = await tx.upload.findMany({
        ...
        return { school, schoolClass, studentsCreated, studentsUpdated };
      });
